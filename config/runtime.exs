...
if config_env() in [:dev, :test] do
  Code.require_file(Path.expand("../lib/manfrod/config/local_env.ex", __DIR__))
  Manfrod.Config.LocalEnv.load!()
end

env = fn key, type, default ->
...
# Zen API (Kimi K2.5)
config :manfrod, :zen_api_key, env.("ZEN_API_KEY", :string?, nil)

# OpenRouter API
config :manfrod, :openrouter_api_key, env.("OPENROUTER_API_KEY", :string?, nil)

# Voyage AI (embeddings + reranking)
config :manfrod, :voyage_api_key, env.("VOYAGE_API_KEY", :string?, nil)

# Groq API (query expansion)
config :manfrod, :groq_api_key, env.("GROQ_API_KEY", :string?, nil)

# Brave Search API (web search)
config :manfrod, :brave_search_api_key, env.("BRAVE_SEARCH_API_KEY", :string?, nil)

# Slack (Socket Mode bot)
config :manfrod, :slack_app_token, env.("SLACK_APP_TOKEN", :string?, nil)
config :manfrod, :slack_bot_token, env.("SLACK_BOT_TOKEN", :string?, nil)

# Google OAuth (Sign-In + Calendar)
config :manfrod, :google_client_id, env.("GOOGLE_CLIENT_ID", :string?, nil)
config :manfrod, :google_client_secret, env.("GOOGLE_CLIENT_SECRET", :string?, nil)
